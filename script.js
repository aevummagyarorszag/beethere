...
function initOutroMessage() {
  ...
  [...message].forEach((character, index) => {
    ...
    if ((index >= 2 && index <= 4) || (index >= 6 && index <= 10)) letter.classList.add('brand-letter');
    letter.textContent = character === ' ' ? '\u00a0' : character;
    letter.style.animationDelay = `${index * 0.085}s`;
    outroMessage.append(letter);
  });
}
...
function setupCarousel(carousel) {
  ...
  if (!returnButton) {
    ...
    returnButton.innerHTML = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20 17a8 8 0 0 0-14-5.3M6 5v6h6"/></svg>';
  ...
  const updateReturnButton = () => {
    ...
    const atEnd = overflow > 36 && carousel.scrollLeft >= overflow - 28;
