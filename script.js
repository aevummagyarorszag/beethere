...
function openEventDetails(event, triggerButton) {
  ...
  if (triggerButton) {
    ...
    const inset = 16;
    const top = Math.min(Math.max(inset, buttonBounds.top - 22), window.innerHeight - dialogBounds.height - inset);
    const left = Math.min(Math.max(inset, buttonBounds.left - dialogBounds.width + buttonBounds.width + 16), window.innerWidth - dialogBounds.width - inset);
...
function renderCard(event, target, { compact = false } = {}) {
  ...
  if (detailsButton) detailsButton.addEventListener('click', () => {
    ...
    openInlineDetails(card, event, detailsButton);
  });
